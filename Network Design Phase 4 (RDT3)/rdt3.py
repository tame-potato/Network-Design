...
def randomCorrupt(pkt):
    ...
    if random()*100 < DATA_CORRUPT_PROB:
        out[0] = 0
        out[1] = 0
    return out
...
def randomCorruptAck(seq):
    ...
    if random()*100 < ACK_CORRUPT_PROB:
        out[0] = 0
        out[1] = 0
    return out
...
def sendRDT(socket, seq, pkt, addr, position = None):
    ...
    makeRDT22(pkt, seq)
    if position[0]+1 == position[1]:   # set timer for last packet
        Start_time = time()
    while True:
        ...
        try:
            ...
            if position[0] + 1 == position[1] and time() - Start_time > 5:
                print('Resending timeout')  # For the last packet if ACK packet has error for 5 sec, then break
                break
